...
/// value. However, there are cases when a single value might have multiple owners. For example, in
/// graph data structures, multiple edges might point to the same node, and that node is
/// conceptually owned by all of the edges that point to it. A node shouldn't be cleaned up unless
/// it doesn't have any edges pointing to it and so has no owners.
///
/// You have to enable multiple ownership explicitly by using the Rust type Rc<T>, which is an
/// abbreviation for reference counting. The Rc<T> type keeps track of the number of references to
/// a value to determine whether or not the value is still in use. If there are zero references to a
...
/// watching it, there would be an uproar from the remaining TV watchers!
///
...
/// Sharing Data
///
/// We'll create a list a that contains 5 and then 10. Then, we'll make two more lists: b that
/// starts with 3 and c that starts with 4. Both the b and c lists will then continue on to the
/// first a list containing 5 and 10. In other words, both lists will share the first list
/// containing 5 and 10.
///
...
///     }
///     Listing 15-17: Demonstrating that we're not allowed to have two lists using Box<T> that try
...
/// every element in the list will live at least as long as the entire list. This is not the case
/// for the elements and lists in Listing 15-17, but not in every scenario.
///
...
/// performance problems in the, we only need to consider the deep-copy clones and can disregard
/// calls to Rc::clone.
///
/// Cloning to Increase the Reference Count
///
/// Let's change our working example in Listing 15-18 so that we can see the reference counts
/// changing as we create and drop references to the Rc<List> in a.
///
/// In Listing 15-19, we'll change main so that it has an inner scope around list c; then, we can
...
/// rather than count because the Rc<T> type also has a weak_count.
///
/// We can see that the Rc<List> in a has an initial reference count of 1; then, each time we call
/// clone, the count goes up by 1. When c goes out of scope, the count goes down by 1. We don't
/// have to call a function to decrease the reference count like we have to call Rc::clone to
/// increase the reference count: The implementation of the Drop trait decreases the reference
...
/// the count is 0, and the Rc<List> is cleaned up completely. Using Rc<T> allows a single value to
/// have multiple owners, and the count ensures that the value remains valid as long as any of the
/// owners still exist.
///
...
/// violate one of the borrowing rules: Multiple borrows to the same place can cause data races and
/// inconsistencies. But being able to mutate data is very useful!
enum List {
    Cons(i32, Rc<List>),
    Nil,
}
