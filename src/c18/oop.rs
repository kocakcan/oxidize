...
///
/// The compiler generates nongeneric implementations of functions and
/// methods for each concrete type that we use in place of a generic
/// type parameter. The code that results from monomorphization is doing
/// static dispatch, which is when the compiler knows what method you're
...
/// In dynamic dispatch cases, the compiler emits code that at runtime will know
/// which method to call.
///
/// When we use trait objects, Rust must use dynamic dispatch. The compiler doesn't know
/// all the types that might be used with the code that's using trait objects, so it
...
/// lookup incurs a runtime cost that doesn't occur with static dispatch. Dynamic dispatch
/// also prevents the compiler from choosing to inline a method's code, which in turn
/// prevents some optimizations, and Rust has some rules about where you can and cannot
/// use dynamic dispatch, called dyn compatibility.
///
/// NOTE: By avoiding monomorphization, trait-object functions will be both faster to compile
/// and produce smaller libraries. However, this usually comes at the cost of runtime
/// performance. Trait objects are rarely easier for developers to work with than trait-
/// bounded generics.
use std::fmt;
